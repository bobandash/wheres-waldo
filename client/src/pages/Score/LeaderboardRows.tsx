...
const PlayerRow:FC<PlayerRowProps> = ({indexPlayerRank, playerName, score, hasSubmittedName}) => {
  return(
    <tr>
...
const NotInLeaderboardRows:FC<DefaultLeaderboardRowsProps> = ({gameInstances, indexPlayerRank, playerName, hasSubmittedName}) =>{
  const {score} = useGameContext();
  return(
    <>
      {gameInstances.map((gameInstance, index) => (
        <DefaultRow key = {gameInstance._id} gameInstance = {gameInstance} index = {index} />
      ))}
      <PlayerRow indexPlayerRank={indexPlayerRank} playerName = {playerName} score = {score} hasSubmittedName = {hasSubmittedName}/>
    </>
  )
}

// component for when the player is in the leaderboard
const InLeaderboardRows:FC<InLeaderboardRowsProps> = ({gameInstances, indexPlayerRank, playerName, hasSubmittedName}) => {
  const {score} = useGameContext();
  return (
    <>
      {gameInstances.map((gameInstance, index) => {
