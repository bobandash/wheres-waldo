...
const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    async function getLeaderboardRank(){
      const data : Array<GameProps> = (await axios.get('/api/game', {
        params: {
          completed: true,
          order: 1,
    ...
    <>
      <div className =  {styles["leaderboard-container"]}>
        ...
        <table className = {styles["table"]}>
          ...
          <tbody>
            {!isInLeaderboard ? 
              <NotInLeaderboardRows hasSubmittedName = {hasSubmittedName} gameInstances = {gameInstances} indexPlayerRank = {indexPlayerRank} playerName = {name} /> :
              <InLeaderboardRows hasSubmittedName = {hasSubmittedName} gameInstances= {gameInstances} indexPlayerRank={indexPlayerRank} playerName= {name} />
            }
          </tbody>
        </table>
